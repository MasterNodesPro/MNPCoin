...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MNPCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MNPCoinMiner : ProcessNewBlock, block not accepted");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    CValidationState state;
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MNPCOINMiner : ProcessBlock, block not accepted");
...
{
    LogPrintf("MNPCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mnpcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running MNPCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
