...
#include "ui_zmnpcontroldialog.h"
...
std::list<std::string> ZMnpControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMnpControlDialog::listMints;

ZMnpControlDialog::ZMnpControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMnpControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMnpControlDialog::~ZMnpControlDialog()
{
    delete ui;
}

void ZMnpControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMnpControlDialog::updateList()
{
...
void ZMnpControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMnpControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
...
void ZMnpControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
