...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MNP);
    ...
    if (!settings.contains("nAnonymizeMNPCoinAmount"))
        settings.setValue("nAnonymizeMNPCoinAmount", 1000);

    nAnonymizeMNPCoinAmount = settings.value("nAnonymizeMNPCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMNPCoinAmount"))
        SoftSetArg("-anonymizemnpcoinamount", settings.value("nAnonymizeMNPCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mnpcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMNPCoinAmount:
            return QVariant(nAnonymizeMNPCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMNPCoinAmount:
            nAnonymizeMNPCoinAmount = value.toInt();
            settings.setValue("nAnonymizeMNPCoinAmount", nAnonymizeMNPCoinAmount);
            emit anonymizeMNPCoinAmountChanged(nAnonymizeMNPCoinAmount);
            break;
        case CoinControlFeatures:
