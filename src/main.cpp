...
#if defined(NDEBUG)
#error "MNPCOIN cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    //////////////// MAINNET ////////////////
        if (nHeight == 0) {
            ...
            nSubsidy = 300000 * COIN;                  // 3 million premine
        } else if (nHeight > 10 && nHeight <= 8000) {
            nSubsidy = 1 * COIN;                        // 7990 block PoW phase with 1 MNP block reward
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MNPCOIN
...
{
    RenameThread("mnpcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMNPMinted()
{
...
void RecalculateZMNPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMNP supply
...
bool RecalculateMNPSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateMNPSupply(Params().Zerocoin_StartHeight());
    }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MNP to zMNP
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MNPCOIN: We use certain sporks during IBD, so check to see if they are
